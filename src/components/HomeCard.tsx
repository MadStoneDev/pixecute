...
export default function HomeCard({ data, type = "main" }: HomeCardProps) {
  if (type === "main") {
    ...
    return (
      ...
      >
        <div
          className={`grid place-content-center w-full h-full bg-white dark:bg-neutral-800 group-hover:bg-rose-600 rounded-xl shadow-lg shadow-neutral-200 dark:shadow-neutral-900 group-hover:text-neutral-100 transition-all duration-300`}
        >
          {secondaryData.icon !== undefined ? secondaryData.icon : null}
        </div>
        <p
          className={`text-sm text-center tracking-wide font-medium text-neutral-400 dark:text-neutral-600 group-hover:text-neutral-900 group-hover:dark:text-neutral-100 transition-all duration-700`}
        >
          {secondaryData.name}
        </p>
      </Link>
